...
from .quantise import Conv1dInteger, LayerNormInteger, LinearInteger, _integer_quantize
...
def build_layer_norm(
    ...
    if encoder_config.get("layernorm") == "quantised":
        quant_config = encoder_config.get("layernorm_config")
        if quant_config is None:
            raise ValueError(
                "encoder_config['layernorm_config'] is required when layernorm='quantised'"
            )
        return LayerNormInteger(normalized_shape, config=quant_config)
    return LayerNorm(normalized_shape)
...
class ResidualAttentionBlock(nn.Module):
    def __init__(
        ...
        self.attn_ln = build_layer_norm(n_state, encoder_config)
        ...
        self.cross_attn_ln = (
            build_layer_norm(n_state, encoder_config) if cross_attention else None
        )
        ...
        self.mlp_ln = build_layer_norm(n_state, encoder_config)
...
class AudioEncoder(nn.Module):
    def __init__(
        ...
        self.register_buffer("positional_embedding", torch.zeros(1500, 384))

        if encoder_config.get("positional_embedding") == "quantised":
            quant_config = encoder_config.get("positional_embedding_config")
            if quant_config is None:
                raise ValueError(
                    "encoder_config['positional_embedding'] is required when positional_embedding='quantised'"
                )
            embedding = _integer_quantize(
                sinusoids(n_ctx, n_state),
                quant_config["data_in_width"],
                quant_config["data_in_frac_width"],
            )
        else:
            embedding = sinusoids(n_ctx, n_state)
        ...
        self.ln_post = build_layer_norm(n_state, encoder_config)
