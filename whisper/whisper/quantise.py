...
class LayerNormInteger(nn.LayerNorm):
    def __init__(
        self,
        normalized_shape,
        eps: float = 1e-5,
    ...
    def forward(self, x: Tensor) -> Tensor:
        ...
        x_q = self.x_quantizer(x.float())
        weight_q = (
            self.w_quantizer(self.weight.float()) if self.weight is not None else None
        )
        bias_q = self.b_quantizer(self.bias.float()) if self.bias is not None else None
